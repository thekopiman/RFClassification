...
class Custom(DataHandler):
    ...
    # The normalization values are targetted for ImageNet100.
    # Have to generate the values ourselves for better results
    common_transforms = [
        transforms.ToTensor(),
    ]
    ...
    def base_dataset(self, data_path, train=True, download=False):
        ...
        print("Loading metadata of ImageNet_{} ({} split).".format(self.custom_size, split))
        metadata_path = os.path.join(
            data_path if self.metadata_path is None else self.metadata_path,
            "{}_{}{}.txt".format(split, self.custom_size, self.suffix)
        )
