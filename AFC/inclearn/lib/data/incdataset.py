...
class DummyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f"HELLLLLLLLLOOOOOOOOO ----------- {type(img)} ----------")
        try:
            # print(np.asarray(img).shape)
            img = resize_with_pad(np.asarray(img), (224,224), (0,0,0))
        except Exception as e:
            print(img)
            print(np.asarray(img), np.asarray(img).shape)
            raise e
        img = self.trsf(Image.fromarray(np.uint8(img)))
...
def resize_with_pad(image: np.array, 
    ...
    new_size = tuple([int(x*ratio) if int(x*ratio) >= 1 else 1 for x in original_shape])
